...
# Load-bearing, do not remove. This file holds the GK-OFF arm; its sibling
# implementing-actor-behaviors-inline.md holds the GK-ON arm. Both must stay
# gated. resolve_and_gate_skill_files runs before name dedup, and against
# SkillHub's gk_enabled/gk_disabled pair an ungated copy is dropped outright
# (assembly.py:600). With all four files toggling one GK the over-loaded-group
# skip fires (assembly.py:568) and every file falls through to the per-file gate
# (assembly.py:659), leaving exactly one project file live per arm — this one
# when the GK is OFF — which project-first dedup then picks over the SkillHub twin.
gk_disabled: horizon_genai_actor_skill_inline_docs
agents:
  - scripting
  - planning
  - skill_recommending
local_tools:
  ...
  - build_assets
  # Step 3 instructs planning and scripting to batch the matched supporting-document
  # paths through this tool. Declared here per the rule above even though both agents
  # already carry it in their base toolset — an instructed-but-undeclared tool is a
  # preventing-skill-overlap/undeclared-tool-usage finding. NOT declared in the
  # -inline variant, whose Step 3 tells the agent not to read those files at all.
  - bulk_read_local_files
consider_skills:
  - name: spawning-npc-actors
    when: a new NPC/enemy type must be created, or an actor template must be placed into the scene
  - name: making-actors-go-to-point
    when: the NPC should walk or move to a specific position or point
  - name: making-actors-wander-idly
    when: the NPC should wander or roam randomly
  - name: making-actors-patrol
    when: the NPC should walk a fixed route or set of waypoints
  - name: making-actors-follow
    when: the NPC should follow, chase, or pursue the player or another entity
  - name: making-actors-flee
    when: the NPC should run away from or avoid a target
  - name: making-actors-lead
    when: the NPC should lead or guide the player to a destination
  - name: configuring-actor-attention
    when: the NPC should look at, face, or track the player with its gaze
  - name: making-actors-search-and-pickup
    when: the NPC should find and pick up a tagged item
  - name: making-actors-deliver-to-tag
    when: the NPC should carry an item to a location and drop it
  - name: making-actors-fetch
    when: the NPC should fetch a specific item and bring it somewhere
  - name: making-actors-use-items
    when: the NPC should use a held item on an object or another entity
  - name: making-actors-attack-when-in-range
    when: the NPC should attack, shoot, cast at, or damage a target that comes within range, without chasing it
  - name: making-actors-engage-in-combat
    when: the NPC should both chase and attack a target — full combat engagement
  - name: implementing-actor-combat
    when: the NPC needs melee combat specifically
  - name: making-actors-die
    when: the NPC should die, despawn, or react when its HP reaches zero
  - name: integrating-actor-health-system
    when: the NPC needs health, HP, or damage of any kind
  ...
  - name: sequencing-actor-behaviors
    when: several behaviors must be chained, sequenced, or cycled in a fixed order, including timed phases
---
...
>
> **This includes facing/rotation.** Do NOT write a per-frame
> `transform.worldRotation` update or a manual `slerp`/`lookAt` loop to turn an
> actor toward a target or its movement direction. Body direction is owned by the
> `ActorBodyDirectionController` (on `ActorTransformMoveComponent`): when a
> behavior is driving rotation the controller re-applies its own target every
> frame and silently overwrites yours, and when none is, a hand-rolled turn still
> bypasses the animation and networking path. Set facing through behavior
> parameters — see "Controlling Actor Facing" in the `working-with-actor-behaviors`
> skill.
...
single authoritative contract: it holds no matter which supporting instructions you read,
...
7. **When spawning an instance** into a scene (not merely authoring the
   template), the spawned entity must be marked **Networkable**. This is a
   spawn-instance step, not a template
...
> **If a supporting instruction below is offered to you as a sub-skill, activate
> it instead of reading its path.** Activating loads the same content, records
> the decision so a later agent inherits it, and pulls in whatever that document
> itself depends on (attacking pulls health; health offers death). Reading the
> path as well only duplicates the content in your context. If a document is not
> offered to you as a sub-skill, read its path as described in Step 3.

| User Intent | Read This Supporting Instruction |
|-------------|---------------------|
...
Use `{{#CONTEXT_PATH}}/sequencing-actor-behaviors/sequencing-actor-behaviors.md`
as the matched supporting instruction for step configuration and compatible behaviors.
...
## Step 3: READ AND APPLY — Load Matched Supporting Instructions

The paths in Steps 0-2 are read targets ONLY for documents that were not offered
to you as sub-skills. `{{#CONTEXT_PATH}}` resolves a path but does not load its
content, so for those, select only the documents whose intent routes match the
request and batch their exact paths with `bulk_read_local_files`. Planning and scripting each read the
matched documents in their own context before planning, delegation, or the first
covered mutation. Do not read the whole directory, and do not treat another
agent's read as content in the current context.

Do not put supporting-document names in `skills_to_activate`. When delegating,
pass the registered parent skill, exact matched paths, and task-critical constraints.
The service-access, initialization-order, and lifecycle rules that apply to all
behaviors are in the **working-with-actor-behaviors** foundation doc, already
inlined above — apply them to every behavior you touch.
